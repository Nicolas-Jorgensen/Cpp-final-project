...
/*vector<Direction>Room::getExits() const
{
    vector<Direction> exitsOfRoom; //we wanna store the directions of the exits
    for(size_t i = 0; i < exits_.size();i++)
    {
        if(exits_.at(i) != nullptr)
        {
            exitsOfRoom.push_back(static_cast<Direction>(i)); //need to convert it into a enum
        }
    }
    return exitsOfRoom;
}
...
// --- facilitators ---
bool Room::connect(Direction exit, Room *r, Direction to) // exit of this room "to" the entrance of *r
{
  ...
  // make connection
  exits_[exit] = r;
  r->exits_[to] = this; //assign the current room (this) to the TO exit of remote room (r)
  return true;
}
